...
Run a KORUS build with a manager that runs one or more builders as subagents. This procedure follows
...
The manager assigns work to its builders and reads their results. Each manager runs within one account; several managers may share a repository.
...
<img src="/assets/diagrams/g04-build-handoffs.svg" alt="A Manager runs one or more Builder subagents. Reviewed pull requests go to the authorized Lander; failed checks reach Regulator." loading="lazy" width="797" height="1027">
</picture>
<figcaption>The Manager runs one or more Builders as subagents, with one brief per task. The Reviewer reads the pull request and returns findings or passes it to the Lander. The authorized Lander sets queue order; the queue merges. Failed checks take the separate Regulator path. <a href="/assets/diagrams/g04-build-handoffs.drawio">Editable diagram</a>.</figcaption>
</figure>
...
| Use builder subagents within the manager's account | Subagents do not need the old console's grant for spawning separate sessions | [Manager card](roles/manager.card.md) |
...
The manager holds its work plan and runs one or more builders as subagents.
...
Write one complete brief per task. Run one or more builders as subagents in your own account.
...
Read each subagent's result before assigning its next task. Confirm that finished work is pushed
...
## 2. Give each builder subagent one brief

The manager starts each builder as a subagent. Give each builder its own worktree and branch.
...
```text
You are a builder subagent. Build the task in your brief as a workflow, then report to your manager.

If the brief leaves something open, do not guess and do not wait for an answer. Write the
question to the manager, comment it on the pull request, and stop.
