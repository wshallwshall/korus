...
| `Stop` | `scripts/hooks/mail-drain.ps1` | -- (hand-wired) | **Consumes** the messages this session actually displayed, and only those: it claims each by an exclusive open, writes a receipt and moves the message out. A discarded session never reaches `Stop`. **Speaks only when it filed something.** Stop fires every turn, so a consumed-nothing line would narrate the normal state into every turn, forever; the empty-box rule in [SESSION-MAIL.md](SESSION-MAIL.md) step 8 serves the `SessionStart` reader, not this one. Every fault path still speaks here. Decides nothing. | fail open, silent unless it filed |
