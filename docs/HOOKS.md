...
| `Stop` | `scripts/hooks/mail-drain.ps1` | -- (hand-wired) | **Consumes** the messages this session actually displayed, and only those: it claims each by an exclusive open, writes a receipt and moves the message out. A discarded session never reaches `Stop`. Decides nothing. | fail open, silent |
